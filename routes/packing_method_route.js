...
const { PackingMethod, validatePackingMethod } = require('../models/packing_method');
const router = express.Router();


router.get('/', auth, async (req, res) => {
    const methods = await PackingMethod.find().sort('name');
    res.json(methods);
});
router.get('/download', [auth,admin], async (req, res) => {
    ...
    const filePath = `public/uploads/files/packingMethod-${dateTime}.csv`;
    let csv;
...
router.post('/', [auth,admin], async (req, res) => {
    const { error } = validatePackingMethod({ name: req.body.name });

    if (error) return res.status(400).json({ error: error.details[0].message });


    let method = await PackingMethod.findOne({ name: req.body.name });
    if (method) return res.status(400).json({ error: 'Packing method already exists with the given name.' });
    ...
    const imagePath = `public/uploads/packingMethod/${filename}.${extension}`;
...
router.put('/:id', [auth,admin], async (req, res) => {
    ...
    if (req.files && Object.keys(req.files).length > 0) {
        ...
        image.mv(imagePath, async function (err) {
            ...
            const method = await PackingMethod.findByIdAndUpdate(req.params.id,
        ...
        const method = await PackingMethod.findByIdAndUpdate(req.params.id, { name: req.body.name, description: req.body.description }, {
            new: true
        });
...
router.delete('/:id', [auth, admin], async (req, res) => {
    const method = await PackingMethod.findByIdAndRemove(req.params.id);

    if (!method) return res.status(404).json({ error: 'The packing method with the given ID was not found.' });
...
router.delete('/deleteMultiple/:id', [auth, admin], async (req, res) => {
    ...
    const product = await PackingMethod.deleteMany({ _id: { $in: ids } });
...
router.get('/:id', auth, async (req, res) => {
    const method = await PackingMethod.findById(req.params.id);
